  ...
  bool pProcess::AttachProcess(const char* ProcessName)
{
  ...
  bool pProcess::AttachProcessHj(const char* ProcessName)
  {
	  ...
	  if (pid_)
	  {
		  ...
		  {
			  std::cout << "[+] SUCCESS: Handle hijacked from another process!" << std::endl;
		  }
		  else
		  {
			  std::cout << "[!] WARNING: Hijack failed. Falling back to NtOpenProcess..." << std::endl;
			  handle_ = OpenProcessNt(PROCESS_QUERY_INFORMATION | PROCESS_VM_OPERATION | PROCESS_VM_READ, FALSE, pid_);

			  if (hj::IsHandleValid(handle_))
				  std::cout << "[+] SUCCESS: Opened handle via NtOpenProcess." << std::endl;
			  else
				  return false;
		  }
